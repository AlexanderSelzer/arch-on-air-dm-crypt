...
That's three partitions. In my case, I forgot to create a separate /boot
partition, because I mixed it up with the Apple bootloader, and everything worked,
but if you want disk encryption that's not good, and there is probably another reason
for separating the partitions like this.
Swap is optional, but in this case (128GB)
